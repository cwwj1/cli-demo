...
const drawRadiusBox = (ctx, {
    ...
    // eslint-disable-next-line @fe-base/es-prototype/no-array-fill
    ctx.fill();
    // 剪切
    ctx.clip();
    if (image) {
        drawImage(ctx, {
            width: imgWidth ? imgWidth : width,
            height: imgHeight ? imgHeight : height,
            x: imgWidth ? ((width - imgWidth) / 2) + x : x,
            y: imgHeight ? ((height - imgHeight) / 2) + y : y,
            image,
        });
    }
    ctx.restore();
...
export const onlineImageToLocalImage = (imgMap) => {
    ...
    return Promise.all(tasks).then(() => {
        console.log('promise. all', resultMap);
        return resultMap;
    })
        .catch((err) => {
            console.log(err);
...
export const getUUID = () => (Date.now() + (100 * (Math.random()).toFixed(2))).toString(16);
